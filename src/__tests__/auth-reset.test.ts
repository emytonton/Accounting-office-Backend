...
describe('GET /auth/reset-password/validate/:token', () => {
  it('should return isFirstAccess=false for password recovery', async () => {
    ...
    const { token } = consoleEmailService.sent[consoleEmailService.sent.length - 1];

    const res = await request(app).get(`/auth/reset-password/validate/${token}`);
    expect(res.status).toBe(200);
  ...
  it('should return 400 for invalid token', async () => {
    const res = await request(app).get('/auth/reset-password/validate/tokeninvalido');
    expect(res.status).toBe(400);
...
describe('POST /auth/reset-password', () => {
  ...
  it('should reject token after single use (RN-014)', async () => {
    await request(app).post('/auth/forgot-password').send({ email: activeUser.identifier });
    const { token } = consoleEmailService.sent[consoleEmailService.sent.length - 1];

    await request(app)
      .post('/auth/reset-password')
      .send({ token, newPassword: 'outraSenha456' });

    const res = await request(app)
      .post('/auth/reset-password')
      .send({ token, newPassword: 'tentandoDeNovo789' });
  ...
  it('should return 400 for invalid token', async () => {
    const res = await request(app)
      .post('/auth/reset-password')
      .send({ token: 'tokeninvalido', newPassword: 'qualquerSenha123' });
    expect(res.status).toBe(400);
  });
  ...
  it('should set password and allow login for first access user', async () => {
    ...
    await request(app)
      .post('/auth/reset-password')
      .send({ token, newPassword: 'primeiraSenha123' });
